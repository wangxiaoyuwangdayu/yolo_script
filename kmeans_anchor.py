...
def compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus):
    ...
    label_files = []
    f = open(label_path)
    for line in f:#islice(f,1,1000):
        #label_path = line.rstrip().replace('images', 'labels')
        #label_path = label_path.replace('JPEGImages', 'labels')
        label_path = line.rstrip().replace('/media/linlin/PROJ2/wangjiayu/darknet/VOCdevkit/VOC2007/JPEGImages', 'E:/log_ceshi/labels')
    ...
    if plus:
        ...
        for centroid_index in centroid_indices:
            centroids.append(boxes[centroid_index])#随机选取9个框作为簇中心
    ...
    for centroid in centroids:
        ...
        print(centroid.w * grid_size * 32, centroid.h * grid_size * 32)
        #print(type(groups))
        #print(np.array(groups).shape)

    #print(groups[1][1].x,groups[1][1].y,groups[1][1].w,groups[1][1].h)
    #print(len(groups[1]))
    return groups,centroids


label_path = "E:/log_ceshi/train_zenguang.txt"
n_anchors = 9
loss_convergence = 1e-6
grid_size = 13
iterations_num = 1000
plus = 0
wang,centers = compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus)
...
colors=['red','blue','purple','gold','green','cyan','lawngreen','darkred','grey']
colors2=['red','blue','black','gold','green','black','black','darkred','grey']
for i in range(9):
    x_w=[];y_h=[]
    for k in range(len(wang[i])):
        #print(wang[i][k].w)
        x_w.append(wang[i][k].w * grid_size * 32)#画出点wang[i][k].w,wang[i][k].h
        y_h.append(wang[i][k].h * grid_size * 32)
    x_all.append(x_w)
    y_all.append(y_h)
    print(len(x_w))
    #ax = fig.add_subplot(1, 1, 1)
    ax.scatter(x_w,y_h,marker = 'o', color = colors[i], s = 0.1,zorder=0)
    #plt.show()
count_color=0
for center in centers:
    rect = plt.Rectangle((0,0),center.w * grid_size * 32,center.h * grid_size * 32,
                        fill=False, edgecolor=colors[count_color], linewidth=1.3)
    rect.set_zorder(1)
    ax.add_patch(rect)
    ax.scatter(center.w * grid_size * 32,center.h * grid_size * 32,marker = 'o', color = 'k', s = 5,zorder=2)
    count_color+=1
#plt.tick_params(labelsize=40)
#plt.tick_params(direction='in')
fig.savefig('E:/log_ceshi/xinzenguang.jpg',dpi=400,bbox_inches='tight')
